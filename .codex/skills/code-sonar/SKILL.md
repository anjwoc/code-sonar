...
- Excalidraw safety: when creating `.excalidraw` or Excalidraw JSON, generate from Mermaid with `scripts/render-excalidraw-from-mermaid.js` when possible. Every arrow must use Arrow Type `직각`, `elbowed: true`, `roundness: null`, port/rail routing, and horizontal/vertical `points`; reject diagonal two-point arrows, non-target node crossings, label-node overlaps, node labels overflowing their boxes, and frame-child padding/gap/center alignment violations. Node text must be bound to its rectangle and centered. Storage/external-resource edges must use restrained protocol colors plus rail offsets and matching labels: `JDBC/JPA` blue, `Redis` rose, `Spring Data` violet, `Elasticsearch`/`index/read` amber.
