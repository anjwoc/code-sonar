...
| `wiki_source_output_dir` | `${SONAR_WIKI_SOURCE_OUTPUT_DIR}` 또는 `${SONAR_OUTPUT_DIR}/resources/wiki` | 수집한 Wiki 원문/요약 캐시 위치 |
...
| `github_output_dir` | `${SONAR_GITHUB_OUTPUT_DIR}` 또는 `${SONAR_OUTPUT_DIR}/resources/github` | GitHub 근거 캐시 위치 |
...
- GitHub 근거는 `resources/github/`와 `_evidence/01. Evidence Ledger.md`에 `github:{repo}:{kind}:{id}` 형식으로 연결한다.
...
| Excalidraw | `sonar/skills/build-excalidraw/SKILL.md` 전체 규칙 적용; CLI: `scripts/render-excalidraw-from-mermaid.js`; Arrow `직각` `elbowed:true`; `roughness:1`; `fontFamily:1` (ExcaliFont); 기본 arrow grayscale + pastel 강조; box palette `#F9F9F9`~`#E0F2F1`; Gap1≥120px Gap2≥100px Gap3 140-170px; label 15px 위 float; bbox 관통/겹침 0건 |
...
| 로컬 디렉터리 | 이유 |
|:---|:---|
| `resources` | 분석 근거 캐시 (wiki, jira, docling, github). Wiki 발행 대상이 아님 |
...
```text
SONAR_OUTPUT_DIR/
├── Index.md                         ← TOC + 통합 그래프 + 읽는 순서
├── 01. {첫번째 문서}.md
├── 02. {두번째 문서}.md
├── ...
├── resources/                       ← 분석 근거 캐시 (Wiki 발행 제외)
│   ├── wiki/
...
│   ├── 02. Scenarios.md
│   └── 03. Cross Project Traceability.md
├── _evidence/
│   ├── 01. Evidence Ledger.md
│   └── 02. Evidence Audit.md
├── _backup/                         ← 제외된 분석 백업
└── {project-a}/                     ← 코드 분석 시 프로젝트별 폴더
...
**문서 번호 규칙:**
- 메인 문서는 읽는 순서대로 `01.`, `02.` ... 접두어를 붙인다.
- `_business/`, `_evidence/` 하위 문서도 동일하게 번호를 붙여 정렬한다.
- `Index.md`는 번호 없이 항상 TOC 역할을 하며 각 문서로의 위키링크를 포함한다.
