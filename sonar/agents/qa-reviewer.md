...
- **Excalidraw Orthogonal Arrows:** `.excalidraw` 또는 Excalidraw JSON 산출물의 모든 arrow는 Arrow Type `직각`이어야 합니다. JSON 기준으로 `elbowed: true`, `roundness: null`, 수평/수직 세그먼트 `points`를 요구하며, 시작점/끝점만 있는 대각선 2-point arrow는 반려합니다. source/target 외 노드 bbox 관통, arrow label과 node bbox 겹침, merge/storage rail 없이 컴포넌트 중앙으로 들어가는 edge도 반려합니다. 저장소 edge가 모두 같은 색이면 반려하고, `JDBC/JPA` blue, `Redis` rose, `Spring Data` violet, `Elasticsearch`/`index/read` amber 계열로 구분되어야 합니다. 단, 색상은 보조 수단이므로 protocol별 rail offset과 label 배치를 함께 사용해야 합니다.
- **Excalidraw Area Layout:** Area frame 내부는 균일한 패딩과 node gap을 가져야 합니다. 기본값은 좌우 `48px`, 상단 `56px`, 하단 `44px`, node gap 가로 `40px`, 세로 `36px`입니다. frame-child padding/gap/center alignment 위반이 있으면 반려합니다.
- **Excalidraw Text Fit:** 노드 라벨이 컴포넌트 박스 밖으로 넘치면 반려합니다. Mermaid `<br/>`/line break는 보존하고, 이벤트/consumer처럼 긴 라벨은 텍스트 기준으로 node width/height를 늘려야 합니다. node text는 rectangle에 binding되고 중앙 정렬되어야 합니다.
- **Index vs Detail:** System Index에는 전체 통합 그래프만 둡니다. 상세 시퀀스, 이벤트 처리, 저장소 의존성, 업무 데이터플로우 그래프는 세부 문서로 이동시키고 Index에는 링크/표 요약만 남깁니다.
- **System Index Ops Inventory:** `resources/wiki`에 인수인계/운영/서버/DB 세팅/모니터링/Fusion/RMS/Swagger/host/pod/TPS 정보가 있는데 System Index에 `운영/서버 인벤토리` 또는 이에 준하는 표 섹션이 없으면 반려합니다. 서버 상세는 그래프에 억지로 넣지 말고 표로 작성해야 합니다.
