...
- **Excalidraw Orthogonal Arrows:** `.excalidraw` 또는 Excalidraw JSON 산출물의 모든 arrow는 Arrow Type `직각`이어야 합니다. JSON 기준으로 `elbowed: true`, `roundness: null`, port/rail routing, 수평/수직 세그먼트 `points`를 요구합니다. 대각선 2-point arrow, source/target 외 노드 bbox 관통, arrow label과 node bbox 겹침, merge/storage rail 없이 컴포넌트 중앙으로 들어가는 edge는 반려합니다. 저장소 edge가 모두 같은 색이면 반려하고, `JDBC/JPA` blue, `Redis` rose, `Spring Data` violet, `Elasticsearch`/`index/read` amber 계열로 구분되어야 합니다. 단, 색상은 보조 수단이므로 protocol별 rail offset과 label 배치를 함께 사용해야 합니다.
