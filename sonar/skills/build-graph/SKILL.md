...
수집 대상:
- `resources/wiki/`의 인수인계, 운영, 서버, DB 세팅, 모니터링, Fusion, RMS, Swagger, host, pod, TPS 관련 페이지
...
Mermaid 그래프를 Excalidraw 또는 `.excalidraw` JSON으로 옮길 때는 **`sonar/skills/build-excalidraw/SKILL.md`** 의 Visual Style Guidelines를 전부 따릅니다. 핵심만 요약:
...
**스타일 핵심:**
- Arrow Type: `직각` — `elbowed: true`, `roundness: null`
- 모든 segment: 수평/수직만 (대각선 금지)
- `roughness: 1` (Medium), `fontFamily: 1` (ExcaliFont)
- Arrow 기본: grayscale (`#868e96`/`#adb5bd`), 프로토콜 강조: pastel palette
- Box palette: `#F9F9F9`, `#F0F4F8`, `#E8F5E9`, `#FFF3E0`, `#F3E5F5`, `#E1F5FE` ...
- Gap 1 (Source→Group): ≥120px / Gap 2 (Component→Component): ≥100px / Gap 3 (Backend→Storage): 140-170px
- Label: 수평 segment 15px 위에 float
- Port: left/right only, 노드 중앙 관통 금지

**QA:** arrow 수 일치, segment 수직/수평, bbox 관통 0, label 겹침 0, text overflow 0, storage edge 프로토콜별 다른 색

자세한 규칙 → `sonar/skills/build-excalidraw/SKILL.md`
