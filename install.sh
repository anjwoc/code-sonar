...
# Claude Code / Codex / Antigravity / Gemini CLI 중 감지된 환경에 자동 설치합니다.
set -uo pipefail
...
# ─── 환경 감지 ───────────────────────────────────────────────────────────────
HAS_CLAUDE=false; HAS_CODEX=false; HAS_ANTIGRAVITY=false; HAS_GEMINI=false
...
$HAS_GEMINI      && ok "Gemini CLI"  || echo -e "  · Gemini CLI   — 미감지 (skip)"

if ! $HAS_CLAUDE && ! $HAS_CODEX && ! $HAS_ANTIGRAVITY && ! $HAS_GEMINI; then
  echo ""
...
# ─── Gemini CLI ──────────────────────────────────────────────────────────────
if $HAS_GEMINI; then
  section "[Gemini CLI] Extension 설치"
  run_step "Gemini CLI" "$SCRIPTS/install-gemini.sh"
fi
